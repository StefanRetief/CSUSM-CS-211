...
#include "Queue.hpp" //your queue class
using namespace std;
...
int main()
{
  ...
  //win is set to true in answerQuestion() - if the user answers quickly and the queue gets empty or he answers 100 questions correctly, the user wins the game
  if(win)
...
void *addQuestion(void* data)
{
    ...
    int waitTime = CLOCKS_PER_SEC * ((level* -1) + 6);
	...
	while(!q.isEmpty() && q.getSize() < 10 && numCorrect != 100)
	  {
          //cout << "ADDQ" << endl;
...
char getOperator(op o)
{
  switch(o) //switch to look at the op enum and return the correct char
    {
    case ADD: return '+';
    case SUB: return '-';
    case MULT: return '*';
...
int correctAnswer(int op1, char optr, int op2)
{
  switch(optr)  //switch to look at the symbol and return the correct
    {           //operation
    case '+': return op1 + op2;
    case '-': return op1 - op2;
    case '*': return op1 * op2;
